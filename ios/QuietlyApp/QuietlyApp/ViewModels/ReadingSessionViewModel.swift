...
final class ReadingSessionViewModel: ObservableObject {
    ...
    @Published var notePageNumber: Int?
    ...
    func addScannedNote(_ text: String, context: ModelContext) {
        guard !text.trimmed.isEmpty else { return }

        // Use current page from endPage if available
        let page = endPage > 0 ? endPage : notePageNumber
