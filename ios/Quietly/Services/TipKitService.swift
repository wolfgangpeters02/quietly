...
struct CaptureQuoteTip: Tip {
    @Parameter
    static var hasStartedSession: Bool = false

    var title: Text {
        Text("Capture Quotes with Your Camera")
    }

    var message: Text? {
        Text("During a reading session, tap the scan button to capture quotes directly from your book using Live Text.")
    }
